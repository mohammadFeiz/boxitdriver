...
import { I_consignment, I_paymentDetail } from "../types";
...
const CodePaymentModal: FC<{ cods: I_consignment[], onPaymentSuccess: () => void, onFailedDelivery: () => void, popup: I_usePopup }> = ({ cods, onPaymentSuccess, onFailedDelivery, popup }) => {
    ...
    const [amounts, setAmounts] = useState<I_paymentDetail>({ product: 0, send: 0, total: 0 })
    const getAmounts = async () => {
        const res = await apis.getCODsAmounts(cods)
        if(res){setAmounts(res)}
    }
    useEffect(() => {
        getAmounts()
    }, [])
    const payment = async () => {
        const res = await apis.codsPayment(cods,amounts.total)
        if (res) {
            popup.addModal({
                header: () => null,
                body: (
    ...
    const unit = 'تومان'
    const row_layout = (text: string, key:keyof I_paymentDetail) => {
        return (
            <div className="flex-row- align-v- fs-12- gap-4-">
                ...
                <div className="bold-">{SplitNumber(amounts[key]) || '0'}</div>
            </div>
        )
    }
    console.log(amounts)
    return (
        <div className="flex-col- h-fit-">
            <div className="flex-col- border-panel m-12- gap-24-">
                {row_layout('هزینه کالا', 'product')}
                {row_layout('هزینه ارسال', 'send')}
                <Splitter dashed={true} />
                {row_layout('مجموع مبلغ قابل پرداخت', 'total')}
            </div>
            <FooterButtons
                trueText="پرداخت"
